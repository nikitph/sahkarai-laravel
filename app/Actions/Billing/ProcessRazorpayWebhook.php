...
class ProcessRazorpayWebhook
{
    ...
    public function handle(string $eventId, array $payload): ProcessedWebhook
    {
        ...
        $webhook = ProcessedWebhook::query()->firstOrCreate(
            ...
            ['event_type' => $eventType, 'payload_hash' => hash('sha256', json_encode($payload, JSON_THROW_ON_ERROR)), 'payload' => $payload],
        );
        ...
        try {
            DB::transaction(function () use ($payload, $eventType, $eventId): void {
