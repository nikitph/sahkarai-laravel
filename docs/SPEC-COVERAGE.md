...
| Archive | 3 | Latest-version browse, 20-row pagination, title/newest sort, source/type/date/tag filters, literal/quoted search, English relevance bias, detail/download |
...
| Ingestion | 7 | Scheduled source adapters, auditable/partial polls, three-failure alerts, canonical originals, SHA dedupe, extraction, revisioning, backfill, generation retries |
| Interpretation | 6 | Structured validated payloads, provenance, locale switch/fallback, issue reporting, terminal failure surface |
...
- `tests/Feature/SahkarAiProductTest.php`: tier access, registration defaults, privacy, credit atomicity/idempotency, context closure, acquisition/revisions, notification eligibility, locale fallback, archive sort/tags, signed subscription/top-up webhooks, proration, and consecutive poll alerts.
