...
The source contract contains 60 Gherkin feature files. TypeScript files alongside them are test-authoring helpers and are intentionally ignored.
...
| Auth and access | 17 | Laravel Fortify sessions, registration/reset, user/admin/organization roles, tier policies, owner-scoped product data, tenant-scoped organization administration |
...
| Payments | 9 | Configured INR plans, Razorpay gateway, signed/idempotent lifecycle and top-up webhooks, organization quantity/discount offers, per-seat credits, proration, queued downgrade/cancel, failure notices, reconciliation |
...
- The earlier organization-dormant scenarios remain historical v1 coverage; `payments/organization_seats.feature` supersedes that dormant state for the bulk-seat surface. The initializer's optional 2FA/passkey security page remains available as a deliberate baseline strengthening.
